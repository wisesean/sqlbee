...
public class SqlQueryBuilder extends AbstractSqlBuilder<SqlQuery> {
   ...
   * @return
   */
  public LeftJoinConfigurer leftJoin(String name, String alias) {
    return this.leftJoin(0, name, alias);
  }
  
  /**
   * 左连接
   * @param idx 连接到被查询表的哪个位置
   * @param name 表名
   * @param alias 表别名
   * @return
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  public LeftJoinConfigurer leftJoin(int idx, String name, String alias) {
    Configurer configurer = new LeftJoinConfigurer();
    configurer.init(this);
    
    this.add(configurer);
    LeftJoinConfigurer leftJoinConfigurer = (LeftJoinConfigurer) configurer;
    leftJoinConfigurer.join(idx, name, alias);
    return leftJoinConfigurer;
  }
   ...
   * @return
   */
  public RightJoinConfigurer rightJoin(String name, String alias) {
    return this.rightJoin(0, name, alias);
  }
   ...
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  public RightJoinConfigurer rightJoin(int idx, String name, String alias) {
    Configurer configurer = new RightJoinConfigurer();
    configurer.init(this);
    
    this.add(configurer);
    RightJoinConfigurer rightJoinConfigurer = (RightJoinConfigurer) configurer;
    rightJoinConfigurer.join(idx, name, alias);
    return rightJoinConfigurer;
  }
}
