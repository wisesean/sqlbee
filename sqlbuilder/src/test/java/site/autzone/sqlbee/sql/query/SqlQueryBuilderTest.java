...
public class SqlQueryBuilderTest {
  ...
  public void testLeftJoinApp() throws Exception {
    SqlQueryBuilder sb = new SqlQueryBuilder().select().cloumn("T1.*").end()
    .from().queryObject("TABLE1", "T1").end().leftJoin("TABLE2", "T2").addBinaryCondition("T1.ID", "T2.ID").end();
    SqlQuery sqlQuery = sb.build();
    Assert.assertEquals("SELECT T1.* FROM TABLE1 T1 LEFT JOIN TABLE2 T2 ON (T1.ID = T2.ID)", sqlQuery.toText());
    
    sb.from().queryObject("TABLE3", "T3").end().leftJoin(1, "TABLE4", "T4").addBinaryCondition("T3.ID", new Value("1")).end();
    sb.binaryCondition("LIKE").left("T1.NAME").right(new Value("test")).end();
    sqlQuery = sb.build();
    sqlQuery.toText();
  }
  ...
  public void testRightJoinApp() throws Exception {
    SqlQueryBuilder sb = new SqlQueryBuilder().select().cloumn("T1.*").end()
    .from().queryObject("TABLE1", "T1").end().rightJoin("TABLE2", "T2").addBinaryCondition("T1.ID", "T2.ID").end();
    SqlQuery sqlQuery = sb.build();
    Assert.assertEquals("SELECT T1.* FROM TABLE1 T1 RIGHT JOIN TABLE2 T2 ON (T1.ID = T2.ID)", sqlQuery.toText());
    
    sb.from().queryObject("TABLE3", "T3").end().rightJoin(1, "TABLE4", "T4").addBinaryCondition("T3.ID", new Value("1")).end();
    sb.binaryCondition("LIKE").left("T1.NAME").right(new Value("test")).end();
    sqlQuery = sb.build();
    sqlQuery.toText();
  }
  ...
  public void testApp() throws Exception {
    ...
    sqlQuery = sb.build();
    Assert.assertEquals(
        "SELECT T1.* FROM TABLE1 T1,TABLE2 T2 WHERE (T1.ID = T2.T1_ID)", sqlQuery.toText());
    Assert.assertEquals(0, sqlQuery.getValues().size());
    ...
    sqlQuery = sb.build();
    Assert.assertEquals(
        "SELECT T1.* FROM TABLE1 T1,TABLE2 T2 WHERE (T1.ID = T2.T1_ID) AND (T1.CATAGORY = ?)",
        sqlQuery.toText());
    ...
    sb.betweenCondition()
        .field("T1.START_DATE")
        .leftValue(new Value(2015, Value.TYPE.INT.name()))
        .rightValue(new Value(2018, Value.TYPE.INT.name()))
        .end();
    sqlQuery = sb.build();
    Assert.assertEquals(
    ...
    sb.inCondition()
        .field("T2.CODE")
        .addValue(new Value("1"))
        .addValue(new Value("2"))
        .addValue(new Value("3"))
        .addValue(new Value("4"))
        .addValue(new Value("5"))
        .addValue(new Value("6"))
        .end();
    sqlQuery = sb.build();
    Assert.assertEquals(
